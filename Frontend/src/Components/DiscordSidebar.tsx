...
import { Server} from "../types/typesDiscordSidebar";
import { Channel } from "../types/typesDiscordSidebar";
import { CategoryProps } from "../types/typesDiscordSidebar";
import { ServerIconProps } from "../types/typesDiscordSidebar";
import { MinimizedChatProps } from "../types/typesDiscordSidebar";
import { DiscordSidebarProps } from "../types/typesDiscordSidebar";
    ...
    return (
      <div className="discord-category">
        <div className="discord-category-header" onClick={onToggle}>
          <span className="discord-category-arrow">{isCollapsed ? '▸' : '▾'}</span>
  ...
  const ServerIcon: React.FC<ServerIconProps> = ({ server, isActive, onClick }) => {
    return (
      ...
      >
        ...
        {server.unreadCount && <div className="discord-unread-badge">{server.unreadCount}</div>}
      </div>
    );
  };
  

  const MinimizedChat: React.FC<MinimizedChatProps> = ({ username, isConnected, onToggleChat }) => {
    return (
      <div className="discord-minimized-chat">
        <div className="discord-user-area">
          ...
          <div className="discord-user-details">
            ...
            <div className="discord-status">{isConnected ? 'Online' : 'Offline'}</div>
          </div>
        </div>
